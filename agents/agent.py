...
class AgentDQN(BaseAgent):
    ...
    def compute_loss(self, batch_vars):
        ...
        batch_state_flag = self.get_sym_flag(batch_state[:,(2*self.act_cardinality+6):(2*self.act_cardinality+self.slot_cardinality)])
        ...
        with torch.no_grad():
            ...
            batch_next_state_flag = self.get_sym_flag(batch_next_state[:,(2*self.act_cardinality+6):(2*self.act_cardinality+self.slot_cardinality)])
    ...
    def run_policy(self, rep, state):
        ...
        if np.random.random() < self.epsilon:
            ...
            if self.warm_start == 1:
                ...
                sym_flag = self.get_sym_flag(rep[:, (2*self.act_cardinality+6):(2*self.act_cardinality+self.slot_cardinality)])
